...
This document summarizes the original brief at `.github/instructions/README.md` from **Assessment Overview** onward. Use it to decide what to build and what not to build. The goal is a focused, maintainable .NET 8 Web API—not a production payment platform.
...
### Confirmed API contract decisions

- A successfully processed payment returns `201 Created`.
- Invalid payment input returns `400 Bad Request` with an RFC 7807 `ValidationProblemDetails` body carrying field-level `errors`, a `traceId`, and a `paymentStatus` extension member set to `Rejected`. It is not a payment envelope: the brief states that no payment could be created, and the payment response `status` field is restricted to `Authorized` or `Declined`.
- The gateway uses standard ASP.NET camelCase JSON. The bank simulator's `snake_case` format is an infrastructure concern for a later adapter.
- An unknown payment ID returns `404 Not Found` with `ProblemDetails`.
- A bank simulator or other dependency failure returns `502 Bad Gateway` with `ProblemDetails` and must not be reported as `Declined`.
- The first implementation slice exposes the contract and documents these responses; payment processing is completed in the later domain, application, and infrastructure steps.
...
- Add useful structured logs for request start/completion, operation, outcome, HTTP status, duration, trace ID, and safe payment identifiers where available.
- Never log request bodies, full card numbers, CVV, authorization headers, secrets, or raw provider payloads.
- Log expected business outcomes at an appropriate information level and unexpected failures once at the API boundary with exception details and correlation data.
...
Implement the solution in small, verifiable steps. Keep each step focused on the current requirement and add tests as the behavior is introduced.

### 1. Confirm the API contract

- Use `POST /payments` to process a payment.
- Use `GET /payments/{id}` to retrieve a payment.
- Define request, success-response, rejected-response, not-found, and simulator-failure models.
- Choose no more than three supported currencies.
- Decide and document the HTTP status codes and response envelopes.

### 2. Define the payment domain

- Create the payment status model: `Authorized`, `Declined`, and `Rejected`.
- Define payment identity and safe payment details.
- Implement validation for card number, expiry, currency, amount, and CVV.
- Ensure invalid input cannot reach the bank simulator.

### 3. Write validation and domain tests

- Add tests for every validation rule and its boundary values.
- Add tests for expired cards, unsupported currencies, invalid amounts, and invalid CVVs.
- Add tests for card masking and the absence of full card data and CVV from safe payment details.

### 4. Implement the application use cases

- Implement a `ProcessPayment` use case.
- Implement a `GetPayment` use case.
- Define small interfaces for the bank simulator client and payment repository.
- Map validation and bank outcomes to explicit application results.

### 5. Implement infrastructure adapters

- Implement a configurable HTTP client for the bank simulator.
- Translate the gateway request to the simulator's `snake_case` request format.
- Map authorized, declined, and `503 Service Unavailable` responses correctly.
- Implement the in-memory or test-double payment repository.

### 6. Implement the API layer

- Add thin controllers or endpoints for payment processing and retrieval.
- Map application results to documented HTTP responses.
- Return only safe, masked payment details.
- Expose and verify the OpenAPI documentation.

### 7. Add integration tests

- Test the HTTP pipeline with `WebApplicationFactory`.
- Test payment creation and retrieval through the API.
- Test the bank-client boundary with a deterministic fake HTTP handler or the supplied simulator.
- Test unknown payment IDs and simulator failures.

### 8. Complete documentation and verification

- Document supported currencies, assumptions, status mappings, and storage decisions.
- Document how to run the API, bank simulator, and tests.
- Run the full test suite and build the solution with warnings visible.
